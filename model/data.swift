...
class MealManager {
    ...
    func logMeal(userId: UUID, mealType: String, image: UIImage, analysis: ClaudeAnalysisResult) -> Meal? {
        ...
        // Update daily stats for calendar view
        DailyStatsManager.shared.updateDailyStats(for: userId)
...
// MARK: - Daily Statistics Manager
class DailyStatsManager {
    static let shared = DailyStatsManager()
    let context = CoreDataManager.shared.context
    
    /// Calculate and store aggregated daily stats (called after each meal log)
    func updateDailyStats(for userId: UUID, date: Date = Date()) {
        let calendar = Calendar.current
        let dayStart = calendar.startOfDay(for: date)
        
        // Get all meals for this day
        guard let dayEnd = calendar.date(byAdding: .day, value: 1, to: dayStart) else { return }
        let meals = MealManager.shared.getMeals(userId: userId, from: dayStart, to: dayEnd)
        
        guard !meals.isEmpty else { return }
        
        // Check if daily stat already exists
        let existingStat = getDailyStat(userId: userId, date: dayStart)
        let dailyStat = existingStat ?? DailyStat(context: context)
        
        if existingStat == nil {
            dailyStat.dailyId = UUID()
            dailyStat.userId = userId
            dailyStat.date = dayStart
        }
        
        // Aggregate by meal type
        let breakfastMeals = meals.filter { $0.mealType?.lowercased() == "breakfast" }
        let lunchMeals = meals.filter { $0.mealType?.lowercased() == "lunch" }
        let dinnerMeals = meals.filter { $0.mealType?.lowercased() == "dinner" }
        
        // Breakfast aggregates
        if !breakfastMeals.isEmpty {
            dailyStat.breakfastMealCount = Int16(breakfastMeals.count)
            dailyStat.breakfastAvgHealthScore = breakfastMeals.map { $0.healthScore }.reduce(0, +) / Float(breakfastMeals.count)
            dailyStat.breakfastAvgPortionScore = breakfastMeals.map { $0.portionQualityScore }.reduce(0, +) / Float(breakfastMeals.count)
            dailyStat.breakfastAvgVarietyScore = breakfastMeals.map { $0.varietyScore }.reduce(0, +) / Float(breakfastMeals.count)
            dailyStat.breakfastAvgBalanceScore = breakfastMeals.map { $0.nutritionBalanceScore }.reduce(0, +) / Float(breakfastMeals.count)
        }
        
        // Lunch aggregates
        if !lunchMeals.isEmpty {
            dailyStat.lunchMealCount = Int16(lunchMeals.count)
            dailyStat.lunchAvgHealthScore = lunchMeals.map { $0.healthScore }.reduce(0, +) / Float(lunchMeals.count)
            dailyStat.lunchAvgPortionScore = lunchMeals.map { $0.portionQualityScore }.reduce(0, +) / Float(lunchMeals.count)
            dailyStat.lunchAvgVarietyScore = lunchMeals.map { $0.varietyScore }.reduce(0, +) / Float(lunchMeals.count)
            dailyStat.lunchAvgBalanceScore = lunchMeals.map { $0.nutritionBalanceScore }.reduce(0, +) / Float(lunchMeals.count)
        }
        
        // Dinner aggregates
        if !dinnerMeals.isEmpty {
            dailyStat.dinnerMealCount = Int16(dinnerMeals.count)
            dailyStat.dinnerAvgHealthScore = dinnerMeals.map { $0.healthScore }.reduce(0, +) / Float(dinnerMeals.count)
            dailyStat.dinnerAvgPortionScore = dinnerMeals.map { $0.portionQualityScore }.reduce(0, +) / Float(dinnerMeals.count)
            dailyStat.dinnerAvgVarietyScore = dinnerMeals.map { $0.varietyScore }.reduce(0, +) / Float(dinnerMeals.count)
            dailyStat.dinnerAvgBalanceScore = dinnerMeals.map { $0.nutritionBalanceScore }.reduce(0, +) / Float(dinnerMeals.count)
        }
        
        // Overall day statistics
        dailyStat.totalMealsLogged = Int16(meals.count)
        let allScores = meals.map { $0.healthScore }
        dailyStat.overallDayScore = allScores.reduce(0, +) / Float(allScores.count)
        
        // Combine all recommendations for daily summary
        let recommendations = meals.compactMap { $0.claudeRecommendations }.filter { !$0.isEmpty }
        dailyStat.dailySummary = recommendations.joined(separator: " ")
        
        CoreDataManager.shared.saveContext()
        print("Updated daily stats for user \(userId) on \(dayStart)")
    }
    
    /// Get daily stat for specific date
    func getDailyStat(userId: UUID, date: Date) -> DailyStat? {
        let calendar = Calendar.current
        let dayStart = calendar.startOfDay(for: date)
        
        let fetchRequest: NSFetchRequest<DailyStat> = DailyStat.fetchRequest()
        fetchRequest.predicate = NSPredicate(
            format: "userId == %@ AND date == %@",
            userId as CVarArg, dayStart as CVarArg
        )
        fetchRequest.fetchLimit = 1
        ...
        do {
            ...
            print("Failed to fetch daily stat: \(error)")
            return nil
        }
    }
    
    /// Get daily stats for a month (for calendar view)
    func getDailyStatsForMonth(userId: UUID, year: Int, month: Int) -> [DailyStat] {
        let calendar = Calendar.current
        ...
        fetchRequest.predicate = NSPredicate(
            ...
            userId as CVarArg, monthStart as CVarArg, monthEnd as CVarArg
        )
        ...
        do {
            ...
            print("Failed to fetch monthly stats: \(error)")
            return []
        }
    }
    
    /// Get all daily stats for user (for history view)
    func getAllDailyStats(userId: UUID) -> [DailyStat] {
        let fetchRequest: NSFetchRequest<DailyStat> = DailyStat.fetchRequest()
        fetchRequest.predicate = NSPredicate(format: "userId == %@", userId as CVarArg)
        ...
        do {
            ...
            print("Failed to fetch all daily stats: \(error)")
            return []
        }
    }
}
